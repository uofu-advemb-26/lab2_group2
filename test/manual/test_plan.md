...
6. Flash the executable to the Pico using:
    ```sh
    cmake --build build --target openocd_test
    ```
7. Open a terminal connection to the Pico using:
    ```sh
    sudo minicom -D /dev/ttyACM0 -b 115200
    ```
8. Follow each step from [Steps and Expected Behavior](#steps-and-expected-behavior) sequentially and compare results to the expected results indicated.
